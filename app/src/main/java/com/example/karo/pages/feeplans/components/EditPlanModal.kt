...
fun EditPlanModal(onSave: (plan: FeePlan) -> Unit, plan: FeePlan) {
    ...
    var year by rememberSaveable { mutableStateOf(plan.year.toString()) }
    var semester by rememberSaveable { mutableStateOf(plan.semester.toString()) }
    var amount by rememberSaveable { mutableStateOf(plan.amount.toString()) }
    var frequency by rememberSaveable { mutableStateOf(plan.frequency.toString()) }
    ...
    Box(
        ...
        Column {
            ...
            Button(
                onClick = { onSave(FeePlan(plan.id, year, semester, amount, frequency)) },
